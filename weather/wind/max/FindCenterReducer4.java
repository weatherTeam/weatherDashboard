		...
		Reducer<Text, Text, Text, Text>
{
			...
			throws IOException
	{
		...
		if (nbrPoint > 1)
		{
			...
			if (radiusMax > 0)
			{
				...
				String centreXString = null; // 6
				String centreYString = null; // 6
				...
				if (centreX < 0)
				{
					if (centreX > -10000)
						...
						centreXString = "-" + Math.abs(centreX);
				}
				else
				{
					if (centreX < 10000)
				...
				if (centreY < 0)
				{
					if (centreY > -10000)
						...
						centreYString = "-" + Math.abs(centreY);
				}
				else
				{
					if (centreY < 10000)
						...
						centreYString = "+" + centreY;
				}

				radiusString = formatRadius(radiusMax);

				if (windMax < 100)
					windMaxString = "0" + windMax;

				rainMaxString = formatRain(rainMax);
				
				if (Integer.parseInt(startDateString.subSequence(0, 6)
						.toString()) == Integer.parseInt(endDateString
						.subSequence(0, 8).toString()))
				{
					// there is an output with + "00" so we have the data by year also
					// and not only by month
					output.collect(new Text(startDateString.subSequence(0, 4)
							.toString() + "00"), new Text(startDateString
							+ endDateString + centreXString + centreYString
							+ radiusString + windMaxString + rainMaxString));
					output.collect(new Text(startDateString.subSequence(0, 6)
							.toString()), new Text(startDateString
							+ endDateString + centreXString + centreYString
							+ radiusString + windMaxString + rainMaxString));
				}
				else
				{
					output.collect(new Text(startDateString.subSequence(0, 4)
							...
							+ endDateString + centreXString + centreYString
							+ radiusString + windMaxString + rainMaxString));
					output.collect(new Text(endDateString.subSequence(0, 4)
							.toString() + "00"), new Text(startDateString
							+ endDateString + centreXString + centreYString
							+ radiusString + windMaxString + rainMaxString));
					output.collect(new Text(startDateString.subSequence(0, 6)
							...
							+ endDateString + centreXString + centreYString
							+ radiusString + windMaxString + rainMaxString));
					output.collect(new Text(endDateString.subSequence(0, 6)
							.toString()), new Text(startDateString
							+ endDateString + centreXString + centreYString
							+ radiusString + windMaxString + rainMaxString));
				}
	...
	public double radius(Point x, Point y)
	{
		...
		double deltaPhi = Math.toRadians((double) y.LAT / 1000.0
				- (double) x.LAT / 1000.0);
		double deltaLambda = Math.toRadians((double) y.LONG / 1000.0
				- (double) x.LONG / 1000.0);

		double a = Math.sin(deltaPhi / 2.0) * Math.sin(deltaPhi / 2.0)
				+ Math.cos(phi1) * Math.cos(phi2) * Math.sin(deltaLambda / 2.0)
	...
	public String formatRadius(double radiusMax)
	{
		String radiusString;
		
		if (radiusMax < 1)
			radiusString = "0000000";
		else if (radiusMax < 10)
			radiusString = "000000" + (int) radiusMax;
		...
		else if (radiusMax < 1000000)
			radiusString = "0" + (int) radiusMax;
		else
			radiusString = (int) radiusMax + "";
		
		return radiusString;
	}

	public String formatRain(int rainMax)
	{
		String rainMaxString;
		...
		if (rainMax < 10)
			...
			rainMaxString = rainMax + "";
		
		return rainMaxString;
	}
}
