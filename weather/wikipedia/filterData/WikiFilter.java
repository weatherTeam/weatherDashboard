...
public class WikiFilter {
	...
	public static String[] keywords = { "blizzard", "cyclone", "hurricane",
			"typhoon", "derecho", "drought", "nor'easter", "storm", "tornado",
			"heat wave", "cold wave", "weather event" };
	...
	public static Pattern categoryRegex = Pattern
			.compile("\\[\\[Category:((\\w|\\s|\\d|')*)(\\|.*)?\\]\\]");

	// Regex to extract title from article
	public static Pattern findTitleRegex = Pattern
			.compile("<title>(.*)</title>");
	 ...
	 */
	public static class WFMap extends MapReduceBase implements
		...
		public void map(LongWritable key, Text value,
			...
			while (matcherCategory.find()) {
				...
				if (matcherKeyword.find()) {
					...
					// We add a random number to the key to split a bit more the
					// work between reducers. (Some categories are much bigger
					// than others)
					int random = (int) (Math.random() * 10);
					output.collect(new Text(matcherKeyword.group(1) + ""
							+ random), new Text(XMLString));
					break;
				}
	...
	* document
	*/
	public static class WFReduce extends MapReduceBase implements
			Reducer<Text, Text, Text, Text> {

		@Override
		public void reduce(Text key, Iterator<Text> articles,
				OutputCollector<Text, Text> output, Reporter reporter)
				throws IOException {

			//Initialize with XML header
	...
	static class MultiFileOutput extends MultipleTextOutputFormat<Text, Text> {
		...
		//This way the key is not stored at the beginning of each file
		@Override
	...
	public static void main(String[] args) throws Exception {
		...
		//Delete existing outputs, because deleting manually each time is annoying after a while.
