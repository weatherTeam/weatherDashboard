...
# Usage:
# The first argument is the folder name on the FTP server which is normally the year
# The second argument is the name of a file listing the files we want to download
...
elif [ -s $2 ] && [ -f $2 ] && [ -r $2 ]; then # file is not empty, is not a directory and is readable
    year="$1"
    filename="$2"
    echo "Downloading..."
    (
