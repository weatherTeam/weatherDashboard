 ...
 * 
 * IDEA
 * The method looks at each user. By looking at his friends we suppose that it exists a triangular relation. So we emit the supposition
 * that A-B-C are in a triangular relation. If indeed this relation exists, then by looking at the friends of B, we will again suppose that
 * this relation exists. And the same for C. If the relation exists, then we will emit the supposition 3 times. In the other case it will be lower.
 * This is how we find triangular relationship.
 * 
 * MAPPER
 * For each userID, it emit a a key = (userID, f1, f2), where f1, f2 are 2 friends of userID. The 3 are ordered by their ID. The value emitted it 1.
 * 
 * REDUCER 1
 * We receive key (userID, f1, f2) and value {1, 1, 1} or {1}. We check if there the relation exists 3 times. If it is the case, then there is 
 * a triangular relation, then we emit a key INTERMED_KEY and a value (1)
 * 
 * REDUCER 2
 * We receive (INTERMED_KEY, {1,1,1,1,1,1,1,1, ... , 1}). By summing all this, we find how many triangular connections exists.
 * 
 * @author Jonathan
 *
 */
...
public class Run {
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
            ...
            final int yearPosition = 13; //-1 because of java substring function: begin index is excluded, last is included
            final int yearPositionEnd = 17;
            final int monthPosition = 17; //-1 because of java substring function: begin index is excluded, last is included
            final int monthPositionEnd = 19;
            final int dayPosition = 19; //-1 because of java substring function: begin index is excluded, last is included
            final int dayPositionEnd = 21;
            final int timePosition = 21; //-1 because of java substring function: begin index is excluded, last is included
...
enum Month {
    ...
    private void value(int v) { month = v; }
